...
import { FilterFamilyService } from '../../../services/filter-family.service';

type FilterStream = [string, boolean];

@Component({
  selector: 'app-filter-auto-family',
  templateUrl: './filter-auto-family.component.html',
  styleUrls: ['./filter-auto-family.component.css'],
  providers: [FilterFamilyService]
})
export class FilterAutoFamilyComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(private ts: TableFamilyService, private fs: FilterFamilyService) {
  ...
  updateAutoComplete(s: string) {
      ...
      p.then(v => v.forEach(x => {
        this.autoComplete.push(x)
      }));
  }
