...
@Component({
  selector: 'app-filter-auto-summary',
  templateUrl: './filter-auto-summary.component.html',
  styleUrls: ['./filter-auto-summary.component.css'],
  providers: [FilterSummaryService]
})
export class FilterAutoSummaryComponent implements OnInit, OnDestroy, AfterViewInit {
  @Input() variants: VariantSummary[];
  @Output() output: EventEmitter<VariantSummary[]> = new EventEmitter();
  show = false;
  selectedIndex = 0;
  currentFilter = '';
  autoComplete: string[] = [];
  ...
  private unfiltered: VariantSummary[];

  constructor(private ts: TableSummaryService, private fs: FilterSummaryService) {
      ...
      this.subscriptions.push(this.filter.debounceTime(120).subscribe(filterStream => {
          ...
          if (this.fs.isCommand(this.currentFilter)) {
              ...
              if (!filter) {
                  ...
                  variants = this.unfiltered.filter((v: VariantSummary) => {
                      let found = false;
